...
def split_data(data: MoleculeDataset,
    ...
    if split_type == 'crossval':
        ...
        return scaffold_split(args, data, sizes=sizes, balanced=True, seed=seed, logger=logger)
...
def scaffold_split(args, data: MoleculeDataset,
                   sizes: Tuple[float, float, float] = (0.8, 0.1, 0.1),
                   balanced: bool = False,
                   seed: int = 0,
    ...
    if args.parser_name == 'finetune' and args.metric == 'screening_metrics':
        act_val = []
        for data_val in val:
            if int(data_val.targets[0]) == 1:
                act_val.append(data_val)
        act_test = []
        for data_test in test:
            if int(data_test.targets[0]) == 1:
                act_test.append(data_test)
        if len(act_val)==0 or len(act_val)==0:
            act_train = []
            for data_train in train:
                if int(data_train.targets[0]) == 1:
                    act_train.append(data_train)
                    if len(act_train)>=4:
                        break
            val.extend(act_train[:2])
            test.extend(act_train[2:4])
