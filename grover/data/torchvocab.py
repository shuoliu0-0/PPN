...
class TorchVocab(object):
    ...
    def __init__(self, counter, max_size=None, min_freq=1, specials=('<pad>', '<other>'), vocab_type='atom'):
        ...
        if vocab_type in ('atom', 'bond', 'graph', 'feature'):
        ...
        for i, smi in enumerate(smiles):
            ...
            if vocab_type == 'atom':
                for atom in mol.GetAtoms():
                    v = atom_to_vocab(mol, atom)
                    sub_counter[v] += 1
            elif vocab_type == 'bond':
                for bond in mol.GetBonds():
                    v = bond_to_vocab(mol, bond)
                    sub_counter[v] += 1
            elif vocab_type == 'graph':
                feature_id = get_atom_feature(mol)
                v = 'acceptor' + str(len(feature_id['acceptor'])) + \
                    '-donor' + str(len(feature_id['donor'])) + \
                    '-NegIonizable' + str(len(feature_id['NegIonizable'])) + \
                    '-PosIonizable' + str(len(feature_id['PosIonizable'])) + \
                    '-Hydrophobe' + str(len(feature_id['Hydrophobe']))
                sub_counter[v] += 1
            else:
                feature_id = get_atom_feature(mol)
                for atom in mol.GetAtoms():
