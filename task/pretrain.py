...
def run_training(args, logger):
    ...
    if master_worker:
        ...
        if args.enable_multi_gpu:
            ...
            print("Total workers: %d" % (mgw.size()))
        debug('Loading data')
    ...
    graph_vocab = MolVocab.load_vocab(args.graph_vocab_path)
    feature_vocab = MolVocab.load_vocab(args.feature_vocab_path)
    atom_vocab_size, bond_vocab_size, feature_vocab_size = len(atom_vocab), len(bond_vocab), len(feature_vocab)

    # Hard coding here, since we haven't load any data yet!
    fg_size = 200
    shared_dict = {}
    mol_collator = GroverCollator(shared_dict=shared_dict, atom_vocab=atom_vocab, bond_vocab=bond_vocab, graph_vocab=graph_vocab, feature_vocab=feature_vocab, args=args)
    if master_worker:
    ...
    for epoch in range(resume_from_epoch + 1, args.epochs):
        ...
        val_av_loss, val_bv_loss, val_fg_loss, _, _, _, val_af_loss = detailed_loss_val
        ...
        if master_worker:
            print('Epoch: {:04d}'.format(epoch),
                  ...
                  'loss_val_af: {:.6f}'.format(val_af_loss),
                  'cur_lr: {:.5f}'.format(trainer.scheduler.get_lr()[0]),
                  't_time: {:.4f}s'.format(t_time),
                  'v_time: {:.4f}s'.format(v_time),
                  'd_time: {:.4f}s'.format(d_time), flush=True)

            writer = SummaryWriter('/apdcephfs/private_sholiu/result/grover/log')
            writer.add_scalar('loss_train', train_loss, epoch)
            writer.add_scalar('loss_val', val_loss, epoch)
            writer.add_scalar('val_av_loss', val_av_loss, epoch)
            writer.add_scalar('val_bv_loss', val_bv_loss, epoch)
            writer.add_scalar('val_fg_loss', val_fg_loss, epoch)
            writer.add_scalar('val_af_loss', val_af_loss, epoch)
