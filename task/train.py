...
def run_training(args: Namespace, time_start, logger: Logger = None) -> List[float]:
    ...
    for model_idx in range(args.ensemble_size):
        ...
        if args.parser_name == 'finetune' and args.metric == 'screening_metrics':
            act_train = []
            inact_train = []
            for data in train_data:
                if int(data.targets[0]) == 1:
                    act_train.append(data)
                else:
                    inact_train.append(data)
        else:
        ...
        for epoch in range(args.epochs):
            if args.parser_name == 'finetune' and args.metric == 'screening_metrics':
                train_dataset = []
                train_dataset.extend(act_train)
                shuffle(inact_train)
                sample = 1*len(act_train)
                train_dataset.extend(inact_train[:sample])
                print('epoch len train_dataset:', len(train_dataset))
                train_data = DataLoader(MoleculeDataset(train_dataset),
                                        batch_size=args.batch_size,
                                        shuffle=shuffles,
                                        num_workers=10,
                                        collate_fn=mol_collator)
            ...
            if args.metric == 'screening_metrics' and isinstance(val_scores[0], dict):
                avg_val_score = np.nanmean(val_scores[0].get("auc"))
            else:
            ...
            if args.show_individual_scores:
                ...
                for task_name, val_score in zip(args.task_names, val_scores):
                    if args.metric == 'screening_metrics':
                        debug(f'Validation {task_name} auc = {val_score.get("auc"):.6f}  ef = {val_score.get("ef"):.6f} f1= {val_score.get("f1"):.6f}')
                    else:
            ...
            if args.metric == 'screening_metrics' and isinstance(val_scores[0], dict):
                ...
                print('Epoch: {:04d}'.format(epoch),
                      ...
                      'auc_val: {:.4f}'.format(val_scores[0].get("auc")),
                      'ef_val: {:.4f}'.format(val_scores[0].get("ef")),
                      'f1_val: {:.4f}'.format(val_scores[0].get("f1")),
                ...
                print('Epoch: {:04d}'.format(epoch),
                      ...
                      'loss_val: {:.6f}'.format(val_loss),
                      f'{args.metric}_val: {avg_val_score:.4f}',
                      #'auc_val: {:.4f}'.format(val_score[0].get("auc")),
                      'cur_lr: {:.5f}'.format(scheduler.get_lr()[-1]),
                      't_time: {:.4f}s'.format(t_time),
                      'v_time: {:.4f}s'.format(v_time))
        ...
        # Average test score
        print(test_scores)
        if args.metric == 'screening_metrics' and isinstance(test_scores[0], dict):
            avg_test_score = np.nanmean(test_scores[0].get("auc"))
        else:
        ...
        if args.show_individual_scores:
            ...
            for task_name, test_score in zip(args.task_names, test_scores):
                if args.metric == 'screening_metrics':
                    info(f'Model {model_idx} test {task_name} auc = {test_score.get("auc"):.6f} ef = {test_score.get("ef"):.6f} f1= {test_score.get("f1"):.6f}')
                else:
        ...
        if args.metric == 'screening_metrics' and isinstance(ensemble_scores[0], dict):
            avg_ensemble_test_score = np.nanmean(ensemble_scores[0].get("auc"))
        else:
        ...
        if args.show_individual_scores:
            for task_name, ensemble_score in zip(args.task_names, ensemble_scores):
                if args.metric == 'screening_metrics':
                    info(f'Ensemble test {task_name} auc = {ensemble_score.get("auc"):.6f}  ef = {ensemble_score.get("ef"):.6f} f1= {ensemble_score.get("f1"):.6f}')
                else:
